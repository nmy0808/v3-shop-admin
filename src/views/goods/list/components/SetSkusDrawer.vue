  ...
  <div>
    ...
    >
      <div v-loading="loading" :class="{'h-screen':loading}">
        <el-form ref="formRef" :model="formData" label-width="80px">
          ...
          <template v-if="formData.sku_type === 0">
            <el-form-item label="市场价格" prop="password">
              <el-input-number
                v-model="formData.sku_value.oprice"
                :precision="2"
              ></el-input-number>
            ...
            <el-form-item label="销售价格" prop="password">
              <el-input-number
                v-model="formData.sku_value.pprice"
                :precision="2"
              ></el-input-number>
              <span class="ml-2">元</span>
            </el-form-item>
            <el-form-item label="成本价格" prop="password">
              <el-input-number
                v-model="formData.sku_value.cprice"
                :precision="2"
              ></el-input-number>
              <span class="ml-2">元</span>
            </el-form-item>
            <el-form-item label="商品重量" prop="password">
              <el-input-number
                v-model="formData.sku_value.weight"
                :precision="2"
              ></el-input-number>
            ...
            <el-form-item label="商品体积" prop="password">
              <el-input-number
                v-model="formData.sku_value.volume"
                :precision="2"
              ></el-input-number>
        ...
        <!-- 多规格 -->
        <SkuCard
          v-if="formData.sku_type === 1"
          v-model:goodsSkus="goods_skus_card"
...
const handleSubmit = async () => {
  try {
    ...
    await goodsSkuUpdateApi({
      ...
      goodsSkus: skuTableRef.value && skuTableRef.value.getData(),
    });
...
const getGoodsSkus = (goodsSkusCard, goodsSkus) => {
  ...
  cloneDeep(goodsSkus).forEach((item) => {
    item._ids = Object.values(item.skus)
      .map((it) => it.id)
      .join(",");
    goodsSkusCopy.push(item);
  });
  const goodsSkusCardCopy = cloneDeep(goodsSkusCard);
  const skus = goodsSkusCardCopy
    .map((it) => [...it.goodsSkusCardValue])
